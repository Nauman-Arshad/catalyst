...
- **Payment method:** `payments.payment_method` is `'cash'` or `'bank'` (check constraint, defaults to `'cash'`). The payment form and the company ledger's "set paid" dialog both set it; the advance inserted by `createOrder` falls back to the default, since the order form has no method picker. `PaymentMethodBadge` in `src/components/payment-method-badge.tsx` renders it with a per-method icon, and `PAYMENT_METHODS` there drives both selects.
...
- **Company ledger** (`/companies`) is for the single supplier company, with one row per day and no company CRUD. A day's bill is derived from that day's orders as `Σ(quantity × company_rate)`. `order_items.company_rate` is snapshotted from `products.company_rate` when an order is created, and `updateOrder` keeps existing snapshots. When a line item has no snapshot, the product's current rate is used, or 0 if it has none. A day's paid figure is derived, never stored: each order's received money (payments linked to it, plus a FIFO share of that party's payments with a null `order_id`) is credited to the company bill capped at it, and the day's paid is the sum of those credits (`allocateReceived` in `companies/_lib/ledger.ts`). **Balance = bill − paid: positive = we owe the company.** `summarizeLedger` nets the whole period rather than summing each day's shortfall, so **Total Pending and Company Balance are the same figure** (a day paid past its bill offsets a day that is short) and the two tiles can never disagree; `advance` is just the negative side of that balance. Per-day rows still use `computeLedgerBalance`, which does floor at zero. **Money paid straight to the company** (`company_payments`, migration `006`) is not an advance and is not tied to a day's bill: `loadCompanyLedger` never reads it, so every *day row* stays derived from orders and party receipts alone. It is still money paid, so its total is passed to `summarizeLedger(days, directPaid)` as the second argument, which adds it to Total Paid, takes it off Total Pending (remainder becomes `advance`, i.e. credit) and so lowers the Company Balance. It is loaded by `loadCompanyPayments`, follows the page's date filter only (never the company-name search), and renders as extra green rows inside the main ledger table (merged by date, the payment under the day that shares its date) with a `paid` status badge, edit + delete icons, and no cap on the amount. `companies/_components/company-payment-buttons.tsx` holds both `AddPaymentButton` and `EditPaymentButton` over one `CompanyPaymentDialog` and one `companyPaymentSchema`; `updateCompanyPayment` takes the row id alongside those fields (`companyPaymentEditSchema`) and updates in place. The page and the PDF (`LedgerPdf`'s `payments` prop) build the same merged row list, so the download matches the screen. `company_ledger_days` is no longer read or written. Data loading is in `companies/_lib/ledger.ts`. Each ledger order also carries `methods`, the distinct `payment_method`s of the payments **linked to that order**; money allocated from the party's unlinked pool belongs to no single method and is deliberately not attributed, so `methods` can under-account for `paid`. The PDF is a real file built with `@react-pdf/renderer` in `companies/pdf/route.ts`. Additive migrations live in `db/migrations/` (apply with `psql -f`; `db:schema` is destructive). The old `companies` / `company_purchases` tables are unused.
